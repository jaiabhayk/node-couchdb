...
exports.createClient = function(port, host, user, pass, maxListeners, secure, cookie) {
  if (isNaN(port)) {
    host = port;
    port = 5984;
  }
...
// return err == 304 if doc hasnt changed
Db.prototype.getDocEtag = function(id, etag, cb) {
  var request = {
      path: '/'+id
  };
  request.headers = {
...
Db.prototype.getRev = function (id, cb) {
    return this.request({
        method: 'HEAD',
...
Db.prototype.removeAttachment = function(docId, attachmentId, docRev, cb) {
  return this.request({
    method: 'DELETE',
    path: '/' + encodeURIComponent(docId) + '/' + attachmentId,
    query: {rev: docRev}
  }, cb);
};
...
Db.prototype.view = function(design, view, query, cb) {
    ...
    var path = ['', ensureDesignId(design), '_view', view].join('/'); 
...
Db.prototype.list = function(design, list, view, query, cb) {
    ...
    var path = ['', ensureDesignId(design), '_list', list, view].join('/');
...
// update handler
// doc_id can be null
Db.prototype.update = function(design, update, doc_id, data, cb) {
    if (typeof(data) === 'function' && !cb) {
        cb = data;
        data = undefined;
    }
    
    var path = ['', ensureDesignId(design), '_update', update].join('/');
    if(doc_id) {
        path += '/' + doc_id;
    }
